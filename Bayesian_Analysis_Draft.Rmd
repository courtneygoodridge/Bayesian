...
bf <- anovaBF(FirstSteeringTime ~ heading, data = magnitudedata, rscaleEffects = c(r = 0.707))
bf
...
*Might delete this*
I set the prior as r = 0.707 which is equivalent to a Cauchy distribution. This a well known default distribution for priors. 
*Might delete this*

The computed Bayes factor is 2.24 x 10^75. This indicates that the current data is 2.24 x 10^75 times more likely under the alternative hypothesis versus the null hypothesis i.e. there is a difference between the levels of my heading factor.

However, am I testing the "right" hypothesis? Richard Morley *ADD LINK TO WESBITE* proposed that testing for differences between your factor levels refers to an *unconstrained model* (unconstrained in the sense that I am looking for any differences between my factor levels without specifiying. However I actually have specific predictions on how my factor levels will differ. This refers to an *order constraint model*:

0.5 < 1.0 < 1.5 < 2.0

My order constraint model will allow me to make 3 comparisons with my Bayes factor:

- Full model versus the null model (using the anovaBF function)
...
The *posterior* function allows us to sample from the posterior distribution from the Bayes factor.

*Mu* usuaully refers to the population 
