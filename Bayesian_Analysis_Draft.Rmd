...
**Problem**: I am unsure how to implement a contrast matrix that would match the constraint model I implemented for the Bayesian ANOVA. Thus this is the closest I can get so far (however I'm sure it is possible).
...
I can then draw from the posterior distributions and plot the HDIs for each of my contrasts. This provides a nice literal interpretation 

"*95% of the the most credible value for standardised effect size of my contrast are not practically equivalent to the null*". 
...
bf_glm <- bayesfactor_parameters(fit_glm, null = c(-.02,.02))
bf_glm
...
Ideally, I would also implement a contrast where I am comparing: *0.5 > 1.0 > 1.5 > 2.0* but as explained above, I'm unsure how to implement that within the contrast matrix. 
