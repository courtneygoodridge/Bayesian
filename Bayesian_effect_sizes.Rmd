...
```{r}
standard_my_model_bayes <- standardize_posteriors(my_model_bayes,
                                                  method = "smart",
                                                  robust = TRUE)

posteriors <- insight::get_parameters(my_model_bayes)
```
...
*posthoc*
posthoc method might be best to use if I do not change headings to factor and just investigate the effect of heading overall.

*smart*
smart method will create effect sizes similar to Glass' Delta because the SD of the intercept is used. Apparently this effect size should be used if the standard deviations sig different (https://www.statisticshowto.datasciencecentral.com/glasss-delta/). Investigate how to work out how difference SDs are. 

*basic*
basic treat all variables as continuous 
