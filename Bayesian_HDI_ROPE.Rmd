...
*contr.treatment* is the default contrast setting. The row of zeros next to 0.5 indicates that R is specifying this as the "control condition". The 1's then indicate what the control condition is being contrasted against i.e. in this example:
...
*contr.helmert* is the contrast setting I have been using for my Bayesian ANOVA models. This contrast setting compares the second level with the first level, compares the third level with the average of the first two and compares the fourth level with the average of the first three. Hence in this example, we would have contrasts such as:

- 0.5 vs 1.0 (heading1)
- 1.5 vs ((0.5 + 1.0) / 2) (heading2)
- 2.0 vs ((0.5 + 1.0 + 1.5) / 2) (heading3)

This *contr.helmert* contrast option is not exactly what I want either. If I am to compare this ANOVA technique versus the anovaBF function, I need 3 contrasts:

- 0.5 vs 1.0
- 1.0 vs 1.5
- 1.5 vs 2.0

These comparisons are not orthogonal, however they represent the research question that I am asking.
...
This plot represents the probability (y axis) of the different effects (x axis). Central values are more probable that those at the tail of the distribution. The different HDIs (heading1, heading2 and heading3) correspond to the different contrasts that are computed under the *contr.helmert* contrast option. 

Heading3 is closer to the ROPE interval because it is contrasting the average of the first three levels with the fourth level. Hence by averaging the first 3 levels, my outcome variable between this and the fourth level are more similar so the resultant effect will be reduced. 
