...
```
The *loo_compare* function compares the two Bayesian model fits using the "leave one out" cross validation method. This method investigates the differences in predictive errors and standard errors for each model. This is encapsulated by the *elpd_diff* (difference in predictive errors) and *se_diff* (difference in standard errors) variables.
